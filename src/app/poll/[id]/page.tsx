...
interface Poll { id: string; title: string; description: string | null; duration_minutes: number; status: string; created_by: string; selected_slot_id: string | null; zoom_join_url: string | null }
interface TimeSlot { id: string; start_time: string; end_time: string; available_count: number; total_responses: number }
interface Participant { id: string; name: string; email: string; has_responded: boolean }
interface Response { slot_id: string; is_available: boolean }
...
export default function VotePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        const { data: pData, error: pErr } = await supabase.from('poll_participants').select('*').eq('token', token).single()
  ...
  const handleSubmit = async () => {
    ...
    try {
      for (const slot of slots) {
        ...
        if (existing) {
          await supabase.from('scheduling_responses').update({ is_available: isAvail }).eq('id', existing.id)
        } else {
          await supabase.from('scheduling_responses').insert({ poll_id: poll.id, participant_id: participant.id, slot_id: slot.id, is_available: isAvail })
        }
      }
      ...
      // Update slot counts
      for (const slot of slots) {
        const { count } = await supabase.from('scheduling_responses').select('*', { count: 'exact', head: true }).eq('slot_id', slot.id).eq('is_available', true)
        const { count: totalCount } = await supabase.from('scheduling_responses').select('*', { count: 'exact', head: true }).eq('slot_id', slot.id)
        await supabase.from('poll_time_slots').update({ available_count: count || 0, total_responses: totalCount || 0 }).eq('id', slot.id)
      }

      // Check if all participants responded
      const { data: allParts } = await supabase.from('poll_participants').select('has_responded').eq('poll_id', poll.id)
      const allResponded = allParts?.every(p => p.has_responded)
      if (allResponded) {
        // Find best slot
        const { data: rankedSlots } = await supabase.from('poll_time_slots').select('*').eq('poll_id', poll.id).order('available_count', { ascending: false }).limit(1)
        if (rankedSlots && rankedSlots.length > 0) {
          await supabase.from('scheduling_polls').update({ status: 'completed', selected_slot_id: rankedSlots[0].id }).eq('id', poll.id)
        }
      }
  ...
  if (poll?.status === 'completed') {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4"><div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 text-center">
  ...
  if (submitted) return (
    <div className="min-h-screen flex items-center justify-center p-4"><div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 text-center">
      ...
      <p className="text-sm text-np-gray/70">You'll receive an email once the meeting is confirmed.</p>
    </div></div>
  )
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
        <div className="flex items-start gap-4">
          <div className="w-12 h-12 bg-np-blue/10 rounded-xl flex items-center justify-center flex-shrink-0">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#476B8E" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>
          </div>
          <div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-100">
          <p className="text-sm text-np-gray">Hi <span className="font-medium text-np-dark">{participant?.name}</span>, tap the times you're available. Green = available.</p>
        ...
        <div key={dk} className="mb-6">
          ...
          <div className="space-y-2">
            {dateSlots.map(slot => {
              const isAvail = responses[slot.id] || false
              return (
                <button key={slot.id} onClick={() => toggleSlot(slot.id)} className={`w-full flex items-center justify-between p-4 rounded-xl border-2 transition-all ${isAvail ? 'bg-np-sage/10 border-np-sage text-np-dark shadow-sm' : 'bg-white border-gray-200 text-np-gray hover:border-gray-300'}`}>
