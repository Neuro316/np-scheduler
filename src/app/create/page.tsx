...
export default function CreatePollPage() {
  ...
  // Quick-add: generate slots for a full day
  const generateDaySlots = (date: string) => {
    ...
    setSlots(prev => [...prev.filter(s => s.date !== date || s.startTime), ...newSlots])
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 text-center">
          ...
          <p className="text-np-gray mb-6">
            Voting links have been emailed to all participants.
            You'll be notified once everyone responds.
          </p>
          <div className="flex gap-3 justify-center">
            <a
              href="/create"
              onClick={() => window.location.reload()}
              className="px-6 py-2 border border-np-blue text-np-blue rounded-xl font-medium
                         hover:bg-np-blue/5 transition-all"
            >
              Create Another
            </a>
            <a
              href="/admin"
              className="px-6 py-2 bg-np-blue text-white rounded-xl font-medium
                         hover:bg-np-blue-dark transition-all"
            >
              View All Polls
            </a>
          </div>
  ...
  return (
    <div className="min-h-screen py-8 px-4">
      <div className="max-w-3xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <a href="/" className="text-sm text-np-gray hover:text-np-blue transition-colors">
            ← Back
          </a>
          <h1 className="text-3xl font-bold text-np-dark mt-2" style={{ fontFamily: 'Cormorant Garamond, serif' }}>
            Create Scheduling Poll
          </h1>
          <p className="text-np-gray mt-1">
            Add time options and participants. Everyone gets a unique voting link.
          </p>
        </div>

        {/* Error */}
        {error && (
          <div className="bg-np-terra/10 border border-np-terra/30 text-np-terra px-4 py-3 rounded-xl mb-6 text-sm">
            {error}
          </div>
        )}
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-np-dark
                           focus:outline-none focus:ring-2 focus:ring-np-blue/30 focus:border-np-blue"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-np-dark
                           focus:outline-none focus:ring-2 focus:ring-np-blue/30 focus:border-np-blue resize-none"
              />
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={addSlot}
              className="text-sm text-np-blue hover:text-np-blue-dark font-medium flex items-center gap-1"
            >
            ...
            {slots.map((slot, i) => (
              <div key={i} className="flex items-center gap-3">
                <input
                  ...
                  className="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-sm text-np-dark
                             focus:outline-none focus:ring-2 focus:ring-np-blue/30"
                />
                <input
                  type="time"
                  value={slot.startTime}
                  onChange={e => updateSlot(i, 'startTime', e.target.value)}
                  className="w-28 px-3 py-2 border border-gray-200 rounded-lg text-sm text-np-dark
                             focus:outline-none focus:ring-2 focus:ring-np-blue/30"
                />
                ...
                {slots.length > 1 && (
                  <button
                    ...
                    className="text-gray-400 hover:text-np-terra transition-colors p-1"
                  >
          ...
          {/* Quick-fill for a date */}
          {slots[0]?.date && (
            <button
              onClick={() => generateDaySlots(slots[0].date)}
              className="mt-3 text-xs text-np-teal hover:text-np-teal/80 font-medium"
            >
              Fill 9am-5pm slots for {slots[0].date}
            </button>
          )}
        </div>
        ...
        <button
          ...
          disabled={creating}
          className="w-full bg-np-blue text-white py-4 rounded-xl font-medium text-lg
